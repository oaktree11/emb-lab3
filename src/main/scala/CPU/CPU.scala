 ...
 */
class CPU(implicit val conf: CPUConfig) extends BaseCPU {
  ...
  //printf("pc: %d instruction %x ins %d ins2 %d\n",pc,instruction,instruction(19,15),instruction(24,20))
  ...
  when ( io.dmem.address === 0x30000000.U && registers.io.readdata2 === 56789.U) {
    r1 := 1.U
    printf("test passed*************** %d\n",io.test_passed)
  }
  when(io.dmem.memread || io.dmem.memwrite) {
    //printf("io.dmem.memwrite %d mem addr %x data %d\n",io.dmem.memwrite,io.dmem.address,registers.io.readdata2)
