...
class CPUTesterDriver(cpuType: String,
  ...
  val hexName = "test.hex"//s"${optionsManager.targetDirName}/${binary}.hex"
println(s" hexname: ",hexName)
 ...
 // println("compiledFirrtl: ",compiledFirrtl)
  ...
  def initRegs(vals: Map[Int, BigInt]) {
    for ((num, value) <- vals) {
      println("reg store num:%d value:%d ",num,value)
  ...
  def dumpAllModules(): Unit = {
    val modules = SingleCycleCPUInfo.getModules()
    for (name <- modules) {
